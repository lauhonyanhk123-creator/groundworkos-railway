...
import { CreateSubcontractorInput, UpdateSubcontractorInput } from "@workspace/api-zod";
...
router.patch("/subcontractors/:id", requireRole("manager"), async (req, res) => {
  ...
  for (const field of requestedFields) {
    if ((allowed as readonly string[]).includes(field)) data[field] = (body as Record<string, unknown>)[field];
  }
